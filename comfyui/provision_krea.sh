...
dl() {  # dl <url> <dest> [auth-header]
  local url="$1" dest="$2" auth="${3:-}"
  if [ -f "$dest" ]; then echo "[provision] exists: $(basename "$dest")"; return; fi
  echo "[provision] downloading $(basename "$dest") ..."
  local args=(-q --show-progress -O "$dest")
  [ -n "$auth" ] && args=(--header="$auth" "${args[@]}")
  wget "${args[@]}" "$url"
}
...
# --- Text encoders (fp8 t5 keeps it comfortable on a 24GB GPU) ---
...
echo "[provision] done. Models in $COMFY/models/{diffusion_models,text_encoders,vae,loras}"
